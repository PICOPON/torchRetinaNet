...
_C.DEVICE.TRAIN_DEVICES = [0] # 训练gpu    0代表第一块gpu, 1 代表第二块gpu, 你可以随意更改. 你可以通过 nvidim-smi 来查看gpu编号及占用情况， 同样的，你可以[0,1,2,3,4,5,6,7]来指定八块gpu 或[0,2,4] 来指定其中的任意三块gpu
_C.DEVICE.TEST_DEVICES = [0]  # 检测gpu

_C.MODEL = CN()
_C.MODEL.BASEMODEL = 'resnet18' # 现支持 resnet18, resnet34, resnet50, resnet101, resnet152
...
_C.TRAIN.BATCH_SIZE = 4        # 训练批次, 如果内存小，可以调小。如果使用多块gpu，请使用整数倍gpu数量的批次数
...
_C.STEP.MODEL_SAVE_STEP = 100  # 训练过程中,模型保存步长
